/**
 * Unit tests for retrospective salience-guided expansion algorithm
 */

import { describe, expect, it } from "vitest";

import type { GraphExpander, Neighbor } from "../../interfaces/graph-expander.js";
import { RetrospectiveSalienceExpansion } from "./retrospective-salience-expansion.js";
...
describe("RetrospectiveSalienceExpansion", () => {
	describe("constructor", () => {
		it("should throw error when no seeds provided", () => {
			const expander = new MockGraphExpander();

			expect(() => new RetrospectiveSalienceExpansion(expander, [])).toThrow(
		...
		it("should accept single seed", () => {
			...
			const expansion = new RetrospectiveSalienceExpansion(expander, ["A"]);
		...
		it("should accept multiple seeds", () => {
			...
			const expansion = new RetrospectiveSalienceExpansion(expander, ["A", "B", "C"]);
	...
	describe("single seed expansion (N=1)", () => {
		...
		it("should remain in Phase 1 when no paths discovered", async () => {
			const expander = new MockGraphExpander();
			expander.addNode("A");
			expander.addNode("B");
			expander.addNode("C");
			...
			// Should behave like degree-prioritised (Phase 1 only)
	...
	describe("bidirectional expansion (N=2)", () => {
		it("should find path between two seeds", async () => {
			...
			const expansion = new RetrospectiveSalienceExpansion(expander, ["A", "C"]);
			const result = await expansion.run();
		...
		it("should handle disconnected seeds", async () => {
			...
			const expansion = new RetrospectiveSalienceExpansion(expander, ["A", "X"]);
			const result = await expansion.run();
		...
		it("should transition to Phase 2 after finding first path", async () => {
			const expander = new MockGraphExpander();
			expander.addNode("A");
			expander.addNode("B");
			expander.addNode("C");
			...
			expander.addEdgeBetween("A", "B");
			expander.addEdgeBetween("B", "C");
			expander.addEdgeBetween("C", "D");
			// Make bidirectional
			expander.addEdgeBetween("B", "A");
			expander.addEdgeBetween("C", "B");
			expander.addEdgeBetween("D", "C");

			const expansion = new RetrospectiveSalienceExpansion(expander, ["A", "D"]);
			const result = await expansion.run();

			// Should have discovered at least one path
			// (Phase 2 should be active after first path)
			expect(result.paths.length).toBeGreaterThanOrEqual(0);
			expect(result.sampledNodes.size).toBeGreaterThan(2);
		});

		it("should prioritize low-degree nodes initially", async () => {
			const expander = new MockGraphExpander();
	...
	describe("multi-seed expansion (N≥3)", () => {
		...
		it("should find multiple paths between seed pairs", async () => {
			const expander = new MockGraphExpander();
			...
			expander.addEdgeBetween("A", "B");
			expander.addEdgeBetween("B", "E");
			expander.addEdgeBetween("A", "C");
			expander.addEdgeBetween("C", "E");

			const expansion = new RetrospectiveSalienceExpansion(expander, ["A", "E"]);
			const result = await expansion.run();

			// May find multiple paths depending on expansion order
			expect(result.paths.length).toBeGreaterThanOrEqual(0);
		});
	});
	...
	describe("phase transition behavior", () => {
		it("should use degree-only priority in Phase 1", async () => {
			const expander = new MockGraphExpander();
			expander.addNode("A", 1);
			expander.addNode("B", 2);
			expander.addNode("C", 3);
			expander.addEdgeBetween("A", "B");
			expander.addEdgeBetween("B", "C");
			...
			// Should complete without errors in Phase 1
			expect(result.sampledNodes.size).toBeGreaterThan(0);
		});

		it("should adapt priority after finding paths", async () => {
			const expander = new MockGraphExpander();
			// Create a graph where Phase 2 would make different decisions
			for (const id of ["A", "B", "C", "D", "E"]) {
				expander.addNode(id);
			}
			expander.addEdgeBetween("A", "B");
			...
			expander.addEdgeBetween("B", "A");
			expander.addEdgeBetween("C", "B");
			expander.addEdgeBetween("E", "C");
			expander.addEdgeBetween("D", "A");
			expander.addEdgeBetween("E", "D");

			const expansion = new RetrospectiveSalienceExpansion(expander, ["A", "E"]);
			const result = await expansion.run();

			// Should have successfully adapted after finding first path
			expect(result.paths.length).toBeGreaterThanOrEqual(0);
			expect(result.sampledNodes.size).toBeGreaterThan(2);
		});
	});
