...
describe("Community Detection: Louvain - Karate Club", () => {
	it("should detect communities with positive modularity", async () => {
		...
		console.log(
			`Karate Club\tLouvain\t${communities.length}\t${totalModularity.toFixed(4)}\t${stats.totalIterations}\t${graph.getNodeCount()}`,
		);
	});
});
