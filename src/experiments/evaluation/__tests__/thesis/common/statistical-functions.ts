 ...
 */
export function mannWhitneyUTest(sampleA: number[], sampleB: number[]): {
	u: number;
	pValue: number;
	significant: boolean;
} {
 ...
 */
export function confidenceInterval(values: number[], confidence = 0.95): { lower: number; upper: number } {
 ...
 */
export function pathDiversity(paths: Array<{ nodes: string[] }>): number {
...
export function calculateTopicCoverage(
	...
	graph: { getDegree(nodeId: string): number; getAllNodeIds(): string[] }
): {
	coverage: number;
	avgDegree: number;
	hubRatio: number;
