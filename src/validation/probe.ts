...
export const validateProbeChordal = (
	...
	const edgeList = _edges.map((edge) => {
...
const checkProbeChordalExhaustive = (
	...
	for (let mask = 0; mask < (1 << vertexCount); mask++) {
		...
		for (let index = 0; index < vertexCount; index++) {
			if (mask & (1 << index)) {
				probeSet.add(vertices[index]);
			} else {
				nonProbeSet.add(vertices[index]);
			}
		}
		...
		const nonProbeArray = [...nonProbeSet];
		for (let index = 0; index < nonProbeArray.length; index++) {
			for (let index_ = index + 1; index_ < nonProbeArray.length; index_++) {
				const u = nonProbeArray[index];
				const v = nonProbeArray[index_];
...
const canMakeChordalByRemovingVertices = (
	...
	for (let toRemove = 1; toRemove <= Math.min(maxVertices, 3); toRemove++) {
		if (tryRemovingVertices(adjacency, vertices, toRemove)) {
			return true;
		}
	}
